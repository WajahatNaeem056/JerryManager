# Toggle default seeding — single source of truth for every toggle's default.

_TOGGLE_DEFAULTS_LIST="toggle_boot_hardening:1 \
    toggle_recovery:1 \
    toggle_rom_spoof:1 \
    toggle_bootloader_spoofer:1 \
    toggle_adb_disabler:0 \
    toggle_adb_disabler_dev_options:0 \
    toggle_adb_disabler_usb_debug:0 \
    toggle_adb_disabler_oem_unlock:0 \
    toggle_rom_fingerprint:0 \
    toggle_suspicious_props:1 \
    toggle_action_gms:1 \
    toggle_action_gms_force_stop:1 \
    toggle_action_gms_clear_data:0 \
    toggle_action_target:1 \
    toggle_auto_target:1 \
    toggle_target_system:0 \
    toggle_teesim_sync:0 \
    toggle_action_security_patch:1 \
    toggle_action_boot_hash:1 \
    toggle_action_pif:1 \
    toggle_lineage_identity_cleanup:0 \
    toggle_nuke_lineage:0 \
    toggle_custom_rom_identity_cleanup:0 \
    toggle_hide_rom_identifier:0 \
    toggle_rom_identifier_cleanup:0 \
    toggle_rom_prefix_cleanup:0 \
    toggle_build_marker_cleanup:0 \
    toggle_pif_traces:1"

seed_toggle_defaults() {
  for _std_pair in $_TOGGLE_DEFAULTS_LIST; do
    _std_key="${_std_pair%%:*}"
    _std_default="${_std_pair#*:}"
    [ -f "$JERRYMANAGER_CONFIG_DIR/$_std_key.val" ] || cfg_set "$_std_key" "$_std_default"
  done
  unset _std_pair _std_key _std_default
}

toggle_default() {
  for _td_pair in $_TOGGLE_DEFAULTS_LIST; do
    case "$_td_pair" in
      "$1:"*)
        echo "${_td_pair#*:}"
        return 0
        ;;
    esac
  done
  echo 1
}

_feature_enabled() {
  [ "$(cfg_get "$1" "$(toggle_default "$1")")" != "0" ]
}
