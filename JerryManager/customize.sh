...
# MODPATH use karo — yeh hamesha sahi path deta hai chahe update ho ya fresh install
CONFIG_DIR="$MODPATH/config"
mkdir -p "$CONFIG_DIR"
_write_default() {
  _key="$1"
  if [ ! -f "$CONFIG_DIR/$_key.val" ]; then
    printf '0' > "$CONFIG_DIR/$_key.val"
  fi
  unset _key
}
_write_default "toggle_recovery"
_write_default "toggle_boot_hardening"
_write_default "toggle_bootloader_spoofer"
_write_default "toggle_rom_spoof"
_write_default "toggle_lsposed"
_write_default "toggle_action_gms"
_write_default "toggle_action_target"
_write_default "toggle_action_security_patch"
_write_default "toggle_action_boot_hash"
_write_default "toggle_action_pif"
